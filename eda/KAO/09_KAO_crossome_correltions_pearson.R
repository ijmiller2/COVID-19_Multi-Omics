...
###### Loading in data from Yuchen, pearson correlations #####

load("P:/All_20200428_COVID_plasma_multiomics/Correlation/cor_4omes_pearson.RData")
...
names_cor <- row.names(cor_4omes_pearson$cor)
...
filter_row <- rowSums(cor_4omes_pearson$adjusted_pvalue[proteins,metabolites_lipids] <0.05)>1
table(filter_row) #154

# Creating a filter for metabolites-lipids where they must have at least one Tau value over 0.4 
filter_col <- colSums(cor_4omes_pearson$adjusted_pvalue[proteins,metabolites_lipids] <0.05)>1
table(filter_col) #246

# significance star
highlight_sig <- cor_4omes_pearson[[3]][proteins,metabolites_lipids] <  0.05
