...
my.formula <- y ~ x
a <- ggplot(proteomics_df, aes(x=Hospital_free_days,y=X7974)) +
  geom_point(aes(color=Age_less_than_90, shape=Gender), size=5) +
  geom_smooth(method='lm', formula= my.formula) +
  stat_poly_eq(formula = my.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
               parse = TRUE) +  
  labs(title = "Linear Model for COVID (1) or NONCOVID (0)\n Gelsolin ~ Hospital Free Days", 
       y = "Gelsolin log2(int.)",
       x = "Hospital Free Days") +
...
linearMod <- lm(X7974 ~ Hospital_free_days, data=covid_df)  # build linear regression model on full data
print(linearMod)
summary(linearMod)

linearMod <- lm(X7974 ~ Hospital_free_days, data=no_covid_df)  # build linear regression model on full data
print(linearMod)
summary(linearMod)

### with confounders / covariates ###
linearMod <- lm(X7974 ~ Hospital_free_days + Age_less_than_90 + Gender, data=covid_df)
print(linearMod)
summary(linearMod)
